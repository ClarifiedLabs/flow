...
func TestFlowsListSummarizesGraphReviewAgents(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = io.WriteString(w, `{"flows":[{"id":"fl-quality","name":"quality","default":true,"start_node":"implement","nodes":[{"id":"fn-1","key":"implement","name":"Implement","kind":"agent","position":0,"config":{"agent":{"agent_def_id":"ad-author","workspace":"change","artifact":"change"}}},{"id":"fn-2","key":"review","name":"Review","kind":"change_review","position":1,"config":{"change_review":{"agents":[{"agent_def_id":"ad-review-default"},{"agent_def_id":"ad-review-advisory","blocking":false}]}}},{"id":"fn-3","key":"verify","name":"Verify","kind":"verify_change","position":2,"config":{"verify_change":{"agents":[{"agent_def_id":"ad-verify-blocking","blocking":true},{"agent_def_id":"ad-verify-advisory-1","blocking":false},{"agent_def_id":"ad-verify-advisory-2","blocking":false}]}}},{"id":"fn-4","key":"done","name":"Done","kind":"terminal","position":3,"config":{"terminal":{"resolution":"completed"}}}]}]}`)
	}))
	t.Cleanup(server.Close)
