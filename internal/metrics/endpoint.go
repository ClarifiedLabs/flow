...
const (
	// ReadHeaderTimeout bounds how long the telemetry server waits for a
	// client to send request headers (slowloris protection).
	ReadHeaderTimeout = 10 * time.Second
	// DefaultShutdownTimeout bounds graceful telemetry endpoint shutdown when
...
// telemetry endpoint. The returned handle controls the listener lifetime. An
...
func StartEndpoint(ctx context.Context, logger *slog.Logger, handler http.Handler, settings Settings) (*Endpoint, error) {
	if !settings.Enabled || handler == nil {
		return &Endpoint{}, nil
	}
	if logger == nil {
