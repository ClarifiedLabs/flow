...
func TestPushBranchPublishesHeadAndIsIdempotent(t *testing.T) {
	...
	if err := PushBranch(ctx, repoPath, "task/t-test-0001"); err != nil {
		t.Fatalf("push branch: %v", err)
	}

	content, present, err := ReadTextFileAtRef(ctx, project.ExchangePath, "refs/heads/task/t-test-0001", "feature.txt")
	if err != nil {
	...
	exchangeHead, err := gitBareOutput(ctx, project.ExchangePath, nil, "rev-parse", "refs/heads/task/t-test-0001")
	if err != nil {
	...
	exchangeHeadAgain, err := gitBareOutput(ctx, project.ExchangePath, nil, "rev-parse", "refs/heads/task/t-test-0001")
	if err != nil {
