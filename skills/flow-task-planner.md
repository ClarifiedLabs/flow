...
For every proposed task, include a focused title, scope and rationale, a concrete description, testable acceptance criteria, relevant tag slugs, and only necessary dependencies expressed through stable local keys. Select implementation flows only from the IDs advertised in the session prompt. Do not schedule generated tasks or create circular dependencies.
