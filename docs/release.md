...
- `ghcr.io/clarifiedlabs/flow-worker` (minimal base image: `flow-worker` and
  `flow` binaries plus the Harness agent CLI and runtime essentials; extend it
...
worktree, runs `go build ./...`, `go vet ./...`, and `make test` (Go tests
isolate their own environment via `internal/testenv`), then
